...
lv_style_t update_setup_heading_style;
...
void update_setup_tile_setup( uint32_t tile_num ) {
    ...
    lv_style_set_bg_color( &update_setup_style, LV_OBJ_PART_MAIN, LV_COLOR_BLACK);
    ...
    lv_style_copy( &update_setup_heading_style, &update_setup_style );
    lv_style_set_text_color( &update_setup_heading_style, LV_OBJ_PART_MAIN, LV_COLOR_WHITE );
    ...
    lv_obj_align( exit_btn, update_setup_tile, LV_ALIGN_IN_TOP_LEFT, 10, 10 );
    lv_obj_set_event_cb( exit_btn, exit_update_check_setup_event_cb );
    
    lv_obj_t *exit_label = lv_label_create( update_setup_tile, NULL);
    lv_obj_add_style( exit_label, LV_OBJ_PART_MAIN, &update_setup_heading_style  );
    lv_label_set_text( exit_label, "update settings");
    lv_obj_align( exit_label, update_setup_tile, LV_ALIGN_IN_TOP_MID, 0, 15 );
